...
cleanUp () {
	...
	rm -f ../bin/hello1.tcl
	rm -f ../bin/hello2.tcl
	rm -f ../bin/json.txt jsondump mdns newudp getglib32 getglib64 *.o Makefile
}
...
${v1}parray light
${v2}puts "\nv2 ...\n"
${v2}set s [ exec cat \$s | ../bin/jsondump | sed "s/'/\"/g"]
${v2}eval [ readYaml [split \$s "\n" ] light "" "" 1]
${v2}parray light
puts "RealRGB red"
puts [realRGB red]
EOF
...
#static="-static"
	...
	if [ $only -ge 3 ]; then
		...
		echo "  compiling color.c ..."    && ./dockcross-armv7  bash -c "\$CC -fPIC -c color.c -o color.o"
		echo "  compiling libTools ..."   && ./dockcross-armv7  bash -c "\$CC -fPIC -std=gnu99 -shared -o libTools.so -DUSE_TCL_STUBS -I$TCLINC hello.c  color.o"
		echo "  compiling colorv1.c ..."  && ./dockcross-armv7  bash -c "\$CC -fPIC -c colorv1.c -o colorv1.o"
		echo "  compiling libToolsv1 ..." && ./dockcross-armv7  bash -c "\$CC -fPIC -std=gnu99 -shared -o libToolsv1.so -DUSE_TCL_STUBS -I$TCLINC hellov1.c  colorv1.o"
	...
	if [  $only -lt 4 ]; then
		scp ../bin/hello1.tcl ../bin/hello2.tcl ../bin/json.txt  ccu:/usr/local/scripts/hue/bin/ 1>/dev/null
ssh ccu /bin/sh <<EOF 
...
if [[ ${para:1:1} =~ u|t|a ]]; then
	...
	if [ $? != 0 ]; then
		ping -c1 -W1 g69ur70c3b1q0lb6.myfritz.net &>/dev/null
		if [ $? != 0 ]; then
...
CROSS1  := /home/ivo/Documents/develop/RaspberryMatic/build-raspmatic_rpi3/host/bin/
CC1     := \$(CROSS1)aarch64-buildroot-linux-gnu-gcc
...
STRIP1  := \$(CROSS1)aarch64-buildroot-linux-gnu-strip --strip-unneeded
...
color:
	...
	\$(CC2) \$(C_FLAGS) -c color.c   -o raspi/color.o
	\$(CC2) \$(C_FLAGS) -c colorv1.c -o raspi/colorv1.o
	\$(CC2) -I. \$(L_FLAGS) raspi/v2/libTools.so hello.c raspi/color.o
	\$(CC2) -I. \$(L_FLAGS) raspi/libTools.so hellov1.c raspi/colorv1.o
	\$(STRIP2) raspi/libTools.so raspi/v2/libTools.so
	@rm -f *.o raspi/*.o

getglib:
	\$(CC) \$(C_FLAGS) getglib.c -o getglib
	\$(STRIP) getglib
	\$(CC1) \$(C_FLAGS) getglib.c -o raspi/getglib64
	\$(STRIP1) raspi/getglib64
...
EOF
	...
	if [  $only -lt 4 ]; then
		scp ../bin/hello1.tcl ../bin/hello2.tcl ../bin/json.txt ${ub}:"Documents/develop/hue/bin/" 1>/dev/null
		scp ../bin/hello1.tcl ../bin/hello2.tcl ../bin/json.txt ${ccu}:"/usr/local/scripts/hue/bin/" 1>/dev/null
ssh ${ub} /bin/sh <<EOF 
echo Ubuntu
echo
